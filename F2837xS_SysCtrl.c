/* ###########################################################################
 *
 * Arquivo F2837xS_SysCtrl.c
 *
 * Funções de inicialização da CPU, incluindo:
 * 			CLock
 * 			PLL
 * 			WathDog
 *
 ...
 * Definições
 */
#define WDT_13110us  0B000
#define WDT_26220us  0B010
#define WDT_52440us  0B011
#define WDT_104880us 0B100
#define WDT_209760us 0B101
#define WDT_419520us 0B110
#define WDT_830400us 0B111
/*
 * Protóripos de funções
 */
void WaychDogInit(void);
...
{
    /*
     * Configuração do módulo WatchDog
     * Quando ativo utilize o comando ServiceDog(); para enviar uma
     * "Good key" e resetar o watchdog
     */
    DisableDog();   //Desabilita o módulo watchdog
    //WaychDogInit(); //Configura e inicia o módulo watchdog (habilitado)
...
// This function must reside in RAM
    ...
    /*Ajuste das entradas analógicas
     * PCLKCR13 controla do clock para o gate das entradas analógicas
     */
	CpuSysRegs.PCLKCR13.bit.ADC_A = 1;   //clock para o gate da entrada analógica A habilitado
	CpuSysRegs.PCLKCR13.bit.ADC_B = 1;   //clock para o gate da entrada analógica B habilitado
	CpuSysRegs.PCLKCR13.bit.ADC_C = 1;   //clock para o gate da entrada analógica C habilitado
	CpuSysRegs.PCLKCR13.bit.ADC_D = 1;   //clock para o gate da entrada analógica D habilitado

    /*Calibração das entradas analógias
     * Ajusta os registradores ANAREFTRIMx :Analog Reference x Register
     */
    if(*((Uint16 *)0x5D1B6) == 0x0000){
        //Dispositivo não calibrado, aplicando valores estáticos de calibração
	...
	CpuSysRegs.PCLKCR13.bit.ADC_A = 0;   //clock para o gate da entrada analógica A desabilitado
	CpuSysRegs.PCLKCR13.bit.ADC_B = 0;   //clock para o gate da entrada analógica B desabilitado
	CpuSysRegs.PCLKCR13.bit.ADC_C = 0;   //clock para o gate da entrada analógica C desabilitado
	CpuSysRegs.PCLKCR13.bit.ADC_D = 0;   //clock para o gate da entrada analógica D desabilitado

	//Ajusta o clock dos módulos PWM
	ClkCfgRegs.PERCLKDIVSEL.bit.EPWMCLKDIV = 1;  //EPWM clock Divide Select, 0->PLLSYSCLK/1 1->PLLSYSCLK/2
	ClkCfgRegs.PERCLKDIVSEL.bit.EMIF1CLKDIV = 0; //0->Sysclk/1 1->Sysclk/2
	ClkCfgRegs.PERCLKDIVSEL.bit.EMIF2CLKDIV = 0; //mesmo
    EDIS;

    /*Inicialização do módulo PLL     *
     *    InitSysPll(Uint16 clock_source, Uint16 imult, Uint16 fmult, Uint16 divsel)
     *    PLLSYSCLK (clock da cpu) = (XTAL_OSC) * (IMULT + FMULT) / (PLLSYSCLKDIV)
     *    clock_source: XTAL_OSC, INT_OSC1, INT_OSC2
     *    imult: 0 à 127
     *    fmult: 0 à 3 (0=>0 1=>0.25 2=>0.50 3=>0.75
     *    Exemplo:
     *        Cristal de 10Mhz: InitSysPll(XTAL_OSC,IMULT_40,FMULT_0,PLLCLK_BY_2);
     *        PLLSYSCLK = 10*(40+0)/2=200Mhz
     */
    InitSysPll(XTAL_OSC,IMULT_40,FMULT_0,PLLCLK_BY_2);
    EALLOW;
    /*
     * Ajusta o clock LSPCLK (Low Speed Per. Clock)
     * LSPCLK = SYSCLK/LSPCLKDIV*
     * *O valor real da divisão é dado por uma tabela: Página. 279
     */
    ClkCfgRegs.LOSPCP.bit.LSPCLKDIV = 0B010; //LSPCLK=/4 :valor padrão em reset.
    EDIS;
    /*
     * Controla o clock dos periféricos
     */
	InitPeripheralClocks();
}

/*			InitPeripheralClocks
 * Essa função inicializa o clock dos periféricos
 * O registrador de controle de clock permite a ativação individual do clock
 * de cada unidade, permitindo desabilitar o clock de periféricos que não
 * estão sendo utilizados com a finalidade de poupar energia
 *
 */
void InitPeripheralClocks()
{
	...
	CpuSysRegs.PCLKCR0.bit.TBCLKSYNC = 0; //Inicia com o clock dos ePWM desligado para possibilitar a sincronização das portadoras
...
/*
 * Inicia o módulo Watchdog, definindo a frequencia de disparo do
 * módulo e mínimo da janela de comparação
 */
void WatchhDogInit(void)
{
	EALLOW;
	WdRegs.WDCR.bit.WDDIS   = 1;           //desabilita o módulo, previne reset
	WdRegs.WDCR.bit.WDCHK   = 0B101;       //
	WdRegs.WDCR.bit.WDPS    = WDT_209760us;//Tempo de disparo do módulo
	WdRegs.WDWCR.bit.MIN    = 0B11111111;  //Liminite mínimo da janela
	WdRegs.WDCR.bit.WDDIS   = 0;           //habilita o módulo, previne reset
	WdRegs.SCSR.bit.WDENINT = 0;           //0->Reseta a CPU, 1->Gera interrupção WDINT
